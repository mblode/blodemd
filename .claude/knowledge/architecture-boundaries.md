...
Edda / blode.md product analytics initialize from `apps/docs/instrumentation-client.ts` (host-gated via `shouldTrackDocsPostHogForHost` in `apps/docs/lib/platform-analytics.ts`) and `apps/web/instrumentation-client.ts` (marketing apex only). They use the **same PostHog project as blode.co** (project 529494). The public write-only token is hardcoded (`PLATFORM_POSTHOG_PROJECT_TOKEN` in `@repo/common`, `posthogKey` in `apps/web/lib/posthog-key.ts`) so a mis-set `NEXT_PUBLIC_POSTHOG_PROJECT_TOKEN` cannot send platform events to a different project. `ui_host` is hardcoded to the US PostHog app, matching blode.co. Local `next dev` hosts do not init the platform SDK. All deployed docs hosts, including customer subdomains, custom domains and path proxies, initialize shared analytics. `isPlatformAnalyticsHost` still restricts signed-in platform identity to product hosts. Events carry `product: edda` and `surface: docs`. Tenant BYO PostHog uses a named `"tenant"` instance in `apps/docs/components/posthog-provider.tsx` so it cannot clobber the platform singleton. If its key matches the shared project, it skips the second instance to avoid duplicate events. Platform ingestion uses the managed reverse proxy at `https://s.blode.md` (`NEXT_PUBLIC_POSTHOG_HOST`, same reverse-proxy pattern as blode.co).
