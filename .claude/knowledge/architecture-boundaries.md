...
Edda / blode.md product analytics initialize from `apps/docs/instrumentation-client.ts` (host-gated via `isPlatformAnalyticsHost` in `apps/docs/lib/platform-analytics.ts`) and `apps/web/instrumentation-client.ts` (marketing apex only). They use the **same PostHog project as blode.co** (project 529494). The public write-only token is hardcoded (`PLATFORM_POSTHOG_PROJECT_TOKEN` in `@repo/common`, `posthogKey` in `apps/web/lib/posthog-key.ts`) so a mis-set `NEXT_PUBLIC_POSTHOG_PROJECT_TOKEN` cannot send platform events to a different project. `ui_host` is hardcoded to the US PostHog app, matching blode.co. Local `next dev` hosts do not init the platform SDK. Customer tenant subdomains and custom domains skip the platform SDK entirely. Tenant BYO PostHog uses a named `"tenant"` instance in `apps/docs/components/posthog-provider.tsx` so it cannot clobber the platform singleton. Platform ingestion uses the managed reverse proxy at `https://s.blode.md` (`NEXT_PUBLIC_POSTHOG_HOST`, same reverse-proxy pattern as blode.co).
