...
- `apps/` — product apps:
  - `apps/web` (Next.js). Owns apex `blode.md`. Apex `/` is a 200 product page that canonicalizes to `https://blode.co/edda` and is noindex,follow so Google does not index `blode.md/` as a duplicate of the blode-co marketing page. Do not 301 `/`, and do not noindex the whole site. About, blog, pricing, docs, dashboard, API, tenant `*.blode.md`, and legal stay on `blode.md`. Rewrites `/docs`, `/api`, `/sites`, `/.well-known`, `/llms*.txt` to `DOCS_APP_URL` and `/app`, `/oauth` to `DASHBOARD_APP_URL`.
