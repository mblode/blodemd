---
title: Introduction
description: "Edda: Knowledge docs for agents. Git-native MDX. Publish on merge. Write MDX in git; the merge publishes the HTML and the Markdown agents fetch."
---

Edda publishes knowledge docs for agents (and humans) on merge. Write MDX in the repo, review it in the pull request, and the merge publishes the HTML plus the Markdown agents fetch. Navigation and branding live in a single `docs.json` file, and `edda push` deploys the same content from the CLI or CI.
