...
<TypeTable
  type={{
    ...
    BLODEMD_PROJECT: {
      description:
        "Project slug. Overrides docs.json `slug`. If `slug` is missing, the CLI temporarily falls back to docs.json `name` for backward compatibility.",
      type: "string",
    },
    BLODEMD_BRANCH: {
      description:
...
| Project        | `--project` | `BLODEMD_PROJECT`        | `slug` in `docs.json`, then legacy fallback to `name`                  |
