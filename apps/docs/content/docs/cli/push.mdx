---
title: edda push
description: Deploy your local MDX docs to Edda with edda push, using a browser session locally or a project deploy key in CI pipelines.
---
...
<TypeTable
  type={{
    ...
    "--api-key <token>": {
      description:
        "Project deploy key for CI. Falls back to the BLODEMD_API_KEY environment variable, then stored credentials from `edda login`.",
      type: "string",
    },
    "--api-url <url>": {
