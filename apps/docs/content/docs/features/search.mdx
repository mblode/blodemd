...
sidebarTitle: Search
description: Edda indexes titles, headings, and body text on deploy so visitors get built-in full-text search with keyboard shortcuts and no extra setup.
---
...
When you deploy with `edda push`, your page titles, headings, and body content are automatically indexed. There is no external search service to configure and no API keys to manage.
