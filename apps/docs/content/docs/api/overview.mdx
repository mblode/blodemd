---
title: REST API overview
sidebarTitle: Overview
description: Use the Edda REST API for programmatic access to projects, deployments, and domains when you need custom tooling beyond the CLI workflows.
---

The base URL for all requests is `https://api.blode.md`.

<Callout type="note">
  The CLI handles deployments automatically. Use the REST API for custom
  integrations or tooling built on top of edda.
</Callout>

## Authentication

Every endpoint uses the Supabase user session token issued by `edda login`:

```
Authorization: Bearer <token>
```

The CLI attaches this header automatically.

## Error format

All errors return JSON with an `error` string and an optional `issues` field for validation errors.

```json
{ "error": "string", "issues": [] }
```

---

## Health

| Method | Path      | Description                       |
| ------ | --------- | --------------------------------- |
| GET    | `/health` | Returns `{ ok: true, timestamp }` |

---

## Projects

| Method | Path                   | Description                                  |
| ------ | ---------------------- | -------------------------------------------- |
| GET    | `/projects/`           | List all projects for the authenticated user |
| POST   | `/projects/`           | Create a project                             |
| GET    | `/projects/:projectId` | Get a project                                |
| PATCH  | `/projects/:projectId` | Update a project                             |

**POST /projects/: request body**

<TypeTable
  type={{
    name: {
      description: "Display name for the project.",
      type: "string",
    },
    slug: {
      description:
        "URL-safe identifier. Used as the subdomain at `{slug}.blode.md`.",
      type: "string",
    },
    description: {
      description: "Optional short description.",
      type: "string",
      default: "undefined",
    },
  }}
/>

Returns the created project.

**PATCH /projects/:projectId: request body**

<TypeTable
  type={{
    name: {
      description: "New display name.",
      type: "string",
      default: "undefined",
    },
    description: {
      description: "New description.",
      type: "string",
      default: "undefined",
    },
    deploymentName: {
      description: "Override the active deployment label.",
      type: "string",
      default: "undefined",
    },
  }}
/>

---

## Deployments

Deployments follow a three-step flow: create → upload files → finalize.

| Method | Path                                               | Description           |
| ------ | -------------------------------------------------- | --------------------- |
| POST   | `/projects/slug/:slug/deployments`                 | Create a deployment   |
| POST   | `/projects/slug/:slug/deployments/:id/files/batch` | Upload files in batch |
| POST   | `/projects/slug/:slug/deployments/:id/finalize`    | Finalize and promote  |

**POST /projects/slug/:slug/deployments: request body**

<TypeTable
  type={{
    branch: {
      description: "Git branch name.",
      type: "string",
      default: '"main"',
    },
    commitMessage: {
      description: "Commit message attached to this deployment.",
      type: "string",
      default: "undefined",
    },
    environment: {
      description: "Deployment environment label.",
      type: "string",
      default: "undefined",
    },
  }}
/>

**POST .../files/batch: request body**

<TypeTable
  type={{
    "files[].path": {
      description: "Relative path of the file within the docs directory.",
      type: "string",
    },
    "files[].contentBase64": {
      description: "Base64-encoded file contents.",
      type: "string",
    },
    "files[].contentType": {
      description: "MIME type. Inferred from the path if omitted.",
      type: "string",
      default: "undefined",
    },
  }}
/>

**POST .../finalize: request body**

<TypeTable
  type={{
    promote: {
      description: "Promote this deployment to production immediately.",
      type: "boolean",
      default: "true",
    },
  }}
/>

---

## Domains

| Method | Path                                                  | Description                  |
| ------ | ----------------------------------------------------- | ---------------------------- |
| GET    | `/projects/:projectId/domains`                        | List domains                 |
| POST   | `/projects/:projectId/domains`                        | Add a custom domain          |
| DELETE | `/projects/:projectId/domains/:domainId`              | Remove a domain              |
| GET    | `/projects/:projectId/domains/:domainId/verification` | Get DNS verification records |
| POST   | `/projects/:projectId/domains/:domainId/verify`       | Trigger domain verification  |

**POST /projects/:projectId/domains: request body**

<TypeTable
  type={{
    hostname: {
      description:
        "The fully-qualified domain name to add, e.g. `docs.example.com`.",
      type: "string",
    },
    pathPrefix: {
      description: "Mount the docs at a subpath, e.g. `/docs`.",
      type: "string",
      default: "undefined",
    },
  }}
/>
