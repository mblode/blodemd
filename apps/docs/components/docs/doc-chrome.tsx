...
const hasSidebar = ({
  ...
  Boolean((nav?.length ?? 0) || (anchors?.length ?? 0) || (tabs?.length ?? 0));

export const DocChromeHeader = ({
  basePath,
  config,
  nav,
  tabs,
...
export const DocChromeSidebar = ({
  ...
  if (!hasSidebar({ anchors, nav, tabs })) {
    return null;
  }
...
export const DocChromeThemeStyle = ({ config }: { config: SiteConfig }) => {
  const css = themeCssFromConfig(config);
  if (!css) {
    return null;
  }
  ...
  return (
    <style
      // oxlint-disable-next-line no-danger -- tenant color tokens scoped to chrome
      dangerouslySetInnerHTML={{ __html: `.doc-chrome{${css}}` }}
    />
  );
};

export const DocChromeBodyScripts = ({ config }: { config: SiteConfig }) => (
...
export const DocChromeFrame = ({
  ...
  style?: CSSProperties;
  theme?: ReactNode;
  scriptsBody?: ReactNode;
  hasDarkLogo?: boolean;
}) => (
  <div
    className="doc-chrome min-h-screen font-sans"
    data-has-dark-logo={hasDarkLogo ? "true" : "false"}
    style={style}
  >
    {theme}
    <a
    ...
    <div className="container-wrapper flex flex-1 flex-col">
      <div
        className="min-h-min flex-1 items-start px-0 [--top-spacing:0] lg:has-[[data-doc-sidebar]:not(:empty)]:grid lg:grid-cols-[var(--sidebar-width)_minmax(0,1fr)] lg:has-[[data-doc-sidebar]:not(:empty)]:[--top-spacing:calc(var(--spacing)*4)]"
        data-doc-chrome-body=""
        style={chromeBodyStyle}
      >
...
export const DocChrome = ({
  ...
  <DocChromeFrame
    hasDarkLogo={Boolean(config.logo?.dark)}
    header={
      <DocChromeHeader
        basePath={basePath}
        config={config}
    ...
    scriptsBody={<DocChromeBodyScripts config={config} />}
    sidebar={
      <DocChromeSidebar
        anchors={anchors}
        basePath={basePath}
